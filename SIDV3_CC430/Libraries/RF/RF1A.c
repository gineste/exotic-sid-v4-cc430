...
// *****************************************************************************
// @fn          ReadRfSettings
// @brief       Read current RF configuration register settings
// @param       RF_SETTINGS *pRfSettings  Pointer to the structure that will holds the rf settings
// @return      none
// *****************************************************************************
void ReadRfSettings(RF_SETTINGS *pRfSettings) {

  pRfSettings->iocfg2     = ReadSingleReg(IOCFG2);
  pRfSettings->iocfg1     = ReadSingleReg(IOCFG1);
  pRfSettings->iocfg0     = ReadSingleReg(IOCFG0);
  pRfSettings->fifothr    = ReadSingleReg(FIFOTHR);
  pRfSettings->sync1      = ReadSingleReg(SYNC1);
  pRfSettings->sync0      = ReadSingleReg(SYNC0);
  pRfSettings->pktlen     = ReadSingleReg(PKTLEN);
  pRfSettings->pktctrl1   = ReadSingleReg(PKTCTRL1);
  pRfSettings->pktctrl0   = ReadSingleReg(PKTCTRL0);
  pRfSettings->addr       = ReadSingleReg(ADDR);
  pRfSettings->channr     = ReadSingleReg(CHANNR);
  pRfSettings->fsctrl1    = ReadSingleReg(FSCTRL1);
  pRfSettings->fsctrl0    = ReadSingleReg(FSCTRL0);
  pRfSettings->freq2      = ReadSingleReg(FREQ2);
  pRfSettings->freq1      = ReadSingleReg(FREQ1);
  pRfSettings->freq0      = ReadSingleReg(FREQ0);
  pRfSettings->mdmcfg4    = ReadSingleReg(MDMCFG4);
  pRfSettings->mdmcfg3    = ReadSingleReg(MDMCFG3);
  pRfSettings->mdmcfg2    = ReadSingleReg(MDMCFG2);
  pRfSettings->mdmcfg1    = ReadSingleReg(MDMCFG1);
  pRfSettings->mdmcfg0    = ReadSingleReg(MDMCFG0);
  pRfSettings->deviatn    = ReadSingleReg(DEVIATN);
  pRfSettings->mcsm2      = ReadSingleReg(MCSM2);
  pRfSettings->mcsm1      = ReadSingleReg(MCSM1);
  pRfSettings->mcsm0      = ReadSingleReg(MCSM0);
  pRfSettings->foccfg     = ReadSingleReg(FOCCFG);
  pRfSettings->bscfg      = ReadSingleReg(BSCFG);
  pRfSettings->agcctrl2   = ReadSingleReg(AGCCTRL2);
  pRfSettings->agcctrl1   = ReadSingleReg(AGCCTRL1);
  pRfSettings->agcctrl0   = ReadSingleReg(AGCCTRL0);
  pRfSettings->worevt1    = ReadSingleReg(WOREVT1);
  pRfSettings->worevt0    = ReadSingleReg(WOREVT0);
  pRfSettings->worctrl    = ReadSingleReg(WORCTRL);
  pRfSettings->frend1     = ReadSingleReg(FREND1);
  pRfSettings->frend0     = ReadSingleReg(FREND0);
  pRfSettings->fscal3     = ReadSingleReg(FSCAL3);
  pRfSettings->fscal2     = ReadSingleReg(FSCAL2);
  pRfSettings->fscal1     = ReadSingleReg(FSCAL1);
  pRfSettings->fscal0     = ReadSingleReg(FSCAL0);
  pRfSettings->fstest     = ReadSingleReg(FSTEST);
  pRfSettings->ptest      = ReadSingleReg(PTEST);
  pRfSettings->agctest    = ReadSingleReg(AGCTEST);
  pRfSettings->test2      = ReadSingleReg(TEST2);
  pRfSettings->test1      = ReadSingleReg(TEST1);
  pRfSettings->test0      = ReadSingleReg(TEST0);
  pRfSettings->partnum    = ReadSingleReg (PARTNUM);
  pRfSettings->version    = ReadSingleReg (VERSION);
  pRfSettings->freqest    = ReadSingleReg (FREQEST);
  pRfSettings->lqi        = ReadSingleReg (LQI);
  pRfSettings->rssi       = ReadSingleReg (RSSI);
  pRfSettings->marcstate  = ReadSingleReg (MARCSTATE);
  pRfSettings->wortime1   = ReadSingleReg (WORTIME1);
  pRfSettings->wortime0   = ReadSingleReg (WORTIME0);
  pRfSettings->pktstatus  = ReadSingleReg (PKTSTATUS);
  pRfSettings->vco_vc_dac = ReadSingleReg (VCO_VC_DAC);
  pRfSettings->txbytes    = ReadSingleReg (TXBYTES);
  pRfSettings->rxbytes    = ReadSingleReg (RXBYTES);
  pRfSettings->rf1aifctl0 = ReadSingleReg (RF1AIFCTL0);
  pRfSettings->rf1aifctl1 = ReadSingleReg (RF1AIFCTL1);
}
